...
import { physiciansApi, specializationsApi, PhysicianSendDto, getAssetUrl } from "@/lib/api";
...
export function EditPhysicianDialog({
  ...
  useEffect(() => {
    if (physician && specializations) {
      ...
      setImage(null);
      const imageUrl = getAssetUrl(physician.image);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (image) {
      ...
      const fetchedImage = await urlToFile(originalImageUrl, "physician-image.jpg");
      if (fetchedImage) {
